...
export default function SurveyEditor() {
  ...
  const [template, setTemplate] = useState<string>(initialTemplate || initialSurveyData?.template || "single-column");
  const [surveyId, setSurveyId] = useState<string | null>(initialSurveyData?.id || id || null);

  useEffect(() => {
    // If we have an ID but no initial data OR initial data is incomplete (e.g. from list view), fetch it
    if (surveyId && (!initialSurveyData || !initialSurveyData.questions || initialSurveyData.questions.length === 0)) {
  ...
  const [qualityIssues, setQualityIssues] = useState<any[]>([]); // New: AI Quality issues
  const [isAuditing, setIsAuditing] = useState(false); // New: Auditing state
  const [showShareDialog, setShowShareDialog] = useState(false); // New: Share dialog

  const previewSections = useMemo(() => {
    const sections: Array<{ title: string, questions: any[] }> = [];
    let currentQ: any[] = [];
    let currentT = "Start"; // Default 'implied' first section

    questions.forEach(q => {
      if (q.type === 'section_header') {
        // Push the accumulated section if it has content OR if it was a named section (not the implicit Start)
        // If it's the implicit "Start" and empty, we likely don't want an empty first page unless explicitly desired.
        if (currentQ.length > 0 || currentT !== "Start") {
          sections.push({ title: currentT, questions: currentQ });
        }
        currentQ = [];
    ...
    // Always push the last segment if it has content or if it's a named section
    if (currentQ.length > 0 || currentT !== "Start") {
      sections.push({ title: currentT, questions: currentQ });
    }

    // Fallback: If absolutely nothing resulted (e.g. empty survey), push one empty Start
    if (sections.length === 0) {
      sections.push({ title: "Start", questions: [] });
    }
  ...
  useEffect(() => {
    if (currentSectionIndex >= previewSections.length && previewSections.length > 0) {
      setCurrentSectionIndex(0);
    }
  }, [previewSections.length, currentSectionIndex]);
  ...
  const handleGenerateOptions = async (index: number) => {
    ...
    try {
      const data = await api.generateOptions(questionText);
  ...
  const handleAuditQuality = async () => {
    ...
    setIsAuditing(true);
    try {
      const result = await api.auditQuality(questions);
      setQualityIssues(result.issues || []);
      if (result.issues?.length > 0) {
        toast({
          title: "Quality Audit Complete",
          description: `Found ${result.issues.length} potential issues to improve.`,
        });
      } else {
        toast({
          title: "High Quality!",
          description: "No bias or leading language detected. Your survey looks great!",
        });
      }
    } catch (e) {
      toast({ title: "Audit Failed", description: "Failed to scan survey quality", variant: "destructive" });
    } finally {
      setIsAuditing(false);
    }
  };

  const applyQuickFix = (index: number, suggestion: string) => {
    updateQuestion(index, 'text', suggestion);
    setQualityIssues(qualityIssues.filter(i => i.index !== index + 1 && i.index !== index));
    toast({ title: "Quick Fix Applied", description: "Question updated to neutral tone." });
  };
  ...
  const handleSave = async () => {
    try {
      ...
      if (surveyId) {
        ...
        result = await api.updateSurvey(surveyId, payload);
      } else {
        // Create new survey
        result = await api.saveSurvey(payload);
      }
  ...
  return (
    <div className="min-h-screen bg-gradient-subtle dark:bg-slate-950 transition-colors duration-500">
      {/* Header */}
      <div className="glass sticky top-0 z-10 shadow-sm border-b border-white/20">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 py-12 px-4">
                      <div className={`mx-auto p-10 transition-all duration-500 ease-in-out ${template === 'minimalist' ? 'max-w-3xl' : // Minimalist: narrower, no shadow/border classes added here (handled below)
                        ...
                        <div className={`text-center space-y-3 mb-12`}>
                          ...
                          <h1 className="text-4xl font-extrabold tracking-tight" style={{ color: design.primaryColor }}>{typeof title === 'string' ? title : (title as any)?.text}</h1>
                          {template !== 'minimalist' && (
                            ...
                            {questions[currentQuestionIndex] && (
                              <div key={currentQuestionIndex} className="space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-500">
                                ...
                                <Label className="text-3xl font-medium block text-gray-800 leading-tight">
                                  {typeof questions[currentQuestionIndex].text === 'string' ? questions[currentQuestionIndex].text : (questions[currentQuestionIndex].text as any)?.text}
                                    ...
                                    <div className="space-y-3 mt-6">
                                      {(questions[currentQuestionIndex].options || ['Option 1', 'Option 2']).map((opt, idx) => (
                                        <div key={idx} className="flex items-center gap-4 p-4 rounded-xl border-2 border-transparent hover:border-gray-200 hover:bg-white/60 cursor-pointer transition-all group">
                                          ...
                                          <span className="text-xl text-gray-600 group-hover:text-gray-900">{typeof opt === 'string' ? opt : (opt as any)?.text}</span>
                                        </div>
                                      ))}
                                    </div>
                                  )}
                          ...
                          <div className="min-h-[400px] flex flex-col justify-between">
                            <div className="space-y-8 animate-in fade-in slide-in-from-right-8 duration-500" key={currentSectionIndex}>
                              <div className="pb-4 border-b border-gray-100">
                                <h2 className="text-2xl font-bold text-gray-800">{typeof previewSections[currentSectionIndex]?.title === 'string' ? previewSections[currentSectionIndex]?.title : (previewSections[currentSectionIndex]?.title as any)?.text}</h2>
                              ...
                              {previewSections[currentSectionIndex]?.questions.map((q, i) => (
                                <div key={i} className="space-y-3">
                                  <Label className="text-lg font-medium text-gray-800 block">
                                    {typeof q.text === 'string' ? q.text : (q as any).text?.text || JSON.stringify(q.text)}
                                    {q.required && <span className="text-red-500 ml-1">*</span>}
                                  </Label>
                                  {/* Simplified inputs for preview */}
                                  {q.type === 'text' && <Input className="bg-white border-2 border-gray-200 focus:border-blue-400 rounded-lg px-4 py-3 shadow-sm hover:shadow-md transition-all" placeholder="Your answer..." disabled />}
                                  {(q.type === 'multiple_choice' || q.type === 'checkboxes') && (
                                    <div className="space-y-3">
                                      {(q.options || ['Option 1', 'Option 2']).map((opt: string, idx: number) => (
                                        <div key={idx} className="flex items-center gap-3 p-3 rounded-lg border-2 border-gray-200 hover:border-blue-300 hover:bg-blue-50/30 transition-all cursor-pointer group">
                                          <div className="w-5 h-5 border-2 rounded-full border-gray-300 group-hover:border-blue-400 transition-colors" />
                                          <span className="text-gray-700 group-hover:text-gray-900 font-medium">{typeof opt === 'string' ? opt : (opt as any)?.text}</span>
                                        </div>
                                      ))}
                                    </div>
                                  )}
                                  {q.type === 'rating' && (
                                    <div className="flex gap-2 mt-2">
                                      {[1, 2, 3, 4, 5].map(i => (
                                        <div key={i} className="w-10 h-10 rounded-full border-2 border-gray-300 flex items-center justify-center text-gray-500 hover:bg-blue-50 hover:border-blue-400 hover:text-blue-600 transition-all cursor-pointer font-semibold">{i}</div>
                                      ))}
                                    </div>
                                  )}
                                  {q.type === 'yes_no' && (
                                    <div className="flex gap-3 mt-2">
                                      <Button variant="outline" className="flex-1 border-2 hover:bg-green-50 hover:border-green-400 transition-all">Yes</Button>
                                      <Button variant="outline" className="flex-1 border-2 hover:bg-red-50 hover:border-red-400 transition-all">No</Button>
                                    </div>
                                  )}
                                </div>
                              ))}
                            ...
                            {questions.map((q, i) => {
                              ...
                              return (
                                <div key={i} className={`space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-700 ${template === 'minimalist' ? 'border-b border-gray-100 pb-12 last:border-0' : ''}`} style={{ animationDelay: `${i * 100}ms` }}>
                                  ...
                                  <div className="pl-6">
                                    {q.type === 'text' && (
                                      <Input className={`bg-white/50 border-2 border-gray-200 focus-visible:ring-0 px-4 py-3 focus-visible:border-blue-400 rounded-lg transition-all shadow-sm hover:shadow-md ${template === 'minimalist' ? 'text-xl' : 'text-lg'}`} placeholder="Type your answer here..." disabled />
                                    )}

                                    {(q.type === 'multiple_choice' || q.type === 'checkboxes') && (
                                      <div className="space-y-3">
                                        {(q.options || ['Option 1', 'Option 2']).map((opt, idx) => (
                                          <div key={idx} className={`flex items-center gap-3 p-4 rounded-lg cursor-pointer transition-all border-2 ${template === 'minimalist' ? 'border-gray-200 hover:bg-blue-50/30 hover:border-blue-300' : 'border-gray-200 hover:bg-blue-50/30 hover:border-blue-300 shadow-sm hover:shadow-md'} group`}>
                                            {q.type === 'multiple_choice' ? (
                                              <div className="w-5 h-5 rounded-full border-2 border-gray-300 group-hover:border-blue-400 transition-colors" />
                                            ) : (
                                              <div className="w-5 h-5 rounded border-2 border-gray-300 group-hover:border-blue-400 transition-colors" />
                                            )}
                                            <span className={`text-gray-700 group-hover:text-gray-900 font-medium ${template === 'minimalist' ? 'text-lg' : 'text-lg'}`}>{typeof opt === 'string' ? opt : (opt as any)?.text}</span>
                                          </div>
                                        ))}
                                      </div>
                                    )}
                                    ...
                                    {q.type === 'rating' && (
                                      <div className="flex gap-3">
                                        {q.ratingType === 'star' ? (
                                          [1, 2, 3, 4, 5].map(i => (
                                            <button key={i} className="group/star transition-transform hover:scale-110">
                                              <Star className="w-10 h-10 text-gray-300 hover:text-yellow-400 fill-transparent hover:fill-yellow-400 transition-all drop-shadow-sm" />
                                            </button>
                                          ))
                                        ) : (
                                          [1, 2, 3, 4, 5].map(i => (
                                            <div key={i} className="w-12 h-12 rounded-full border-2 border-gray-300 flex items-center justify-center text-gray-500 hover:bg-blue-50 hover:text-blue-600 hover:border-blue-400 transition-all cursor-pointer font-semibold shadow-sm hover:shadow-md hover:scale-105">{i}</div>
                                          ))
                                        )}
                                      </div>
                                    )}
                                    ...
                                    {q.type === 'yes_no' && (
                                      <div className="flex gap-4">
                                        <Button variant="outline" className="flex-1 border-2 border-gray-300 hover:bg-green-50 hover:border-green-400 hover:text-green-700 transition-all shadow-sm hover:shadow-md">Yes</Button>
                                        <Button variant="outline" className="flex-1 border-2 border-gray-300 hover:bg-red-50 hover:border-red-400 hover:text-red-700 transition-all shadow-sm hover:shadow-md">No</Button>
                                      </div>
                                    )}
                                  </div>
                                </div>
              ...
              <Button
                ...
                disabled={isAuditing}
                className={qualityIssues.length > 0 ? "border-amber-500 bg-amber-50" : ""}
              >
                {isAuditing ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Sparkles className="w-4 h-4 mr-2 text-amber-500" />}
                Audit Quality
              </Button>
              <Button
                variant="outline"
              ...
              <Button
                ...
                onClick={() => {
                  if (!surveyId) {
                    toast({ title: "Save Required", description: "Please save your survey first before distributing", variant: "destructive" });
                    return;
                  }
      ...
      <Dialog open={showShareDialog} onOpenChange={setShowShareDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Copy the link below to distribute your survey.
            </DialogDescription>
          </DialogHeader>
          <div className="flex items-center space-x-2 py-4">
            <div className="grid flex-1 gap-2">
              ...
              <Input
                id="link"
                value={`${window.location.protocol}//${window.location.hostname}:8080/survey/${surveyId}`}
                readOnly
                className="bg-muted"
              />
            </div>
            <Button
              ...
              className="px-3"
              onClick={() => {
                const url = `${window.location.protocol}//${window.location.hostname}:8080/survey/${surveyId}`;
          ...
          <div className="text-sm text-muted-foreground bg-blue-50 p-4 rounded-lg border border-blue-100 mb-2">
            <p className="flex items-center gap-2 text-blue-800 font-medium mb-1">
              <Shield className="w-4 h-4" />
              Quality Control Active
            </p>
            <p className="text-blue-700">
              {requireQualification
                ? "This survey requires respondents to pass the qualification test before entry."
                : "This survey is currently open to everyone. You can enable Quality Control in the editor."}
            </p>
          </div>
                      ...
                      <div className="flex-1 space-y-4">
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            {/* NEW: AI Bias Warning */}
                            {qualityIssues.find(issue => (issue.index === index + 1 || issue.index === index)) && (
                              <div className="mt-2 p-3 bg-amber-50 border border-amber-200 rounded-lg animate-in slide-in-from-top-2">
                                <div className="flex items-start gap-2">
                                  ...
                                  <div className="flex-1">
                                    <p className="text-sm font-semibold text-amber-900">AI Quality Warning: {qualityIssues.find(i => i.index === index + 1 || i.index === index)?.type}</p>
                                    <p className="text-xs text-amber-800 mt-1">{qualityIssues.find(i => i.index === index + 1 || i.index === index)?.reason}</p>
                                    <div className="mt-2 flex items-center justify-between gap-4 p-2 bg-white rounded border border-amber-100">
                                      <span className="text-xs italic text-gray-600">"{qualityIssues.find(i => i.index === index + 1 || i.index === index)?.suggestion}"</span>
                                      <Button
                                        size="sm"
                                        variant="ghost"
                                        className="h-7 text-xs text-amber-700 hover:text-amber-800 hover:bg-amber-100 shrink-0"
                                        onClick={() => applyQuickFix(index, qualityIssues.find(i => i.index === index + 1 || i.index === index)?.suggestion)}
                                      >
                                        Apply Fix
                                      </Button>
                                    </div>
                            ...
                            <Input
                              ...
                              className={`mt-2 font-medium ${q.type === 'section_header' ? 'text-xl border-primary/50 bg-primary/5' : ''} ${qualityIssues.find(issue => issue.index === index + 1 || issue.index === index) ? 'border-amber-300 ring-2 ring-amber-100' : ''}`}
                ...
                <div className="p-4 bg-gradient-to-br from-indigo-50 to-purple-50 rounded-lg border border-indigo-100 space-y-3">
                  ...
                  <div className="relative">
                    ...
                    <div className="relative flex justify-center text-xs uppercase">
                      <span className="bg-gradient-to-br from-indigo-50 to-purple-50 px-2 text-indigo-400">Or paste URL</span>
                    </div>
                  </div>
