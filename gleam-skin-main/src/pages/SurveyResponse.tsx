...
import { Sparkles, CheckCircle2, Shield, Star, ChevronRight, ChevronLeft, Mic, Printer, Clock } from "lucide-react";
...
const SurveyResponse = () => {
  ...
  // ... (inside fetchSurvey data processing)
      ...
      const fetchSurvey = async () => {
        try {
          ...
          if (data.require_qualification) {
            ...
            const testData = await api.getQualificationTests(id);
            const tests = Array.isArray(testData) ? testData : (testData.results || []);
            if (tests.length > 0) {
              setQualificationQuestions(tests[0].questions);
              // Set timer if exists
              if (tests[0].time_limit && tests[0].time_limit > 0) {
                setTimeLimit(tests[0].time_limit);
                setTimeLeft(tests[0].time_limit * 60);
                // Only start timer if we are showing qualification
                if (data.allowed_domains && data.allowed_domains.length === 0) {
                  setTimerActive(true);
                }
              }
  ...
  const sections = useMemo(() => {
    ...
    if (template === 'page-by-page') {
      // Each question is its own section
    ...
    if (template === 'sectional') {
      const hasHeaders = survey.questions.some((q: any) => q.type === 'section_header');
      if (!hasHeaders) {
        return [{ title: "Survey", questions: survey.questions }];
      }
    ...
    // Default: 'single-column' or 'minimalist' - all in one section
    return [{
      title: survey.title || "Survey",
      questions: survey.questions.filter((q: any) => q.type !== 'section_header')
    }];
  }, [survey, template]);
  ...
  useEffect(() => {
    ...
    if (timerActive && timeLeft > 0 && showingQualification) {
      interval = setInterval(() => {
        setTimeLeft((prev) => {
          if (prev <= 1) {
            ...
            setQualificationPassed(false); // Auto fail
            toast({
              title: "Time's Up!",
              description: "You exceeded the time limit for the qualification test.",
              variant: "destructive"
            });
            return 0;
          }
  ...
  }, [timerActive, timeLeft, showingQualification, toast]);
  ...
  // ... (Update handleEmailSubmit to start timer)
  const handleEmailSubmit = (e: React.FormEvent) => {
    ...
    if (isAllowed) {
      ...
      if (requireQualification) {
        setShowingQualification(true);
        if (timeLimit > 0) setTimerActive(true); // Start timer here
      }
      toast({
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <Card className="max-w-md w-full shadow-lg">
          <CardHeader>
            ...
            <CardDescription>The requested survey could not be found.</CardDescription>
          </CardHeader>
          <CardContent>
            <Link to="/">
  ...
  if (showingQualification && !qualificationPassed) {
    // If time ran out (timeLeft === 0 and timeLimit > 0), show failure screen
        ...
        <div className="min-h-screen bg-gradient-subtle flex items-center justify-center p-4">
          <Card className="max-w-md w-full shadow-lg border-red-200">
            <CardHeader className="text-center">
              ...
              <CardTitle className="text-red-700">Time Limit Exceeded</CardTitle>
              <CardDescription>
                You did not complete the qualification test within the {timeLimit} minute limit.
              </CardDescription>
            </CardHeader>
    ...
    return (
      <div className="min-h-screen bg-gradient-subtle py-16 px-4">
        <Card className="max-w-4xl mx-auto shadow-lg relative overflow-hidden">
          {timeLimit > 0 && (
            <div className={`absolute top-0 left-0 right-0 h-1.5 ${timeLeft < 60 ? 'bg-red-500' : 'bg-blue-500'} transition-all`} style={{ width: `${(timeLeft / (timeLimit * 60)) * 100}%` }} />
          )}
          <CardHeader className="text-center relative">
            <CardTitle>Qualification Test</CardTitle>
            {timeLimit > 0 && (
              <div className={`absolute top-4 right-4 flex items-center gap-2 font-mono text-lg font-bold ${timeLeft < 60 ? 'text-red-600 animate-pulse' : 'text-blue-600'}`}>
            ...
            <form onSubmit={handleQualificationSubmit} className="space-y-6">
              {qualificationQuestions.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground border-2 border-dashed rounded-lg">
                  ...
                  <p className="text-sm mt-2">The survey requires qualification, but the test appears to be empty.</p>
                </div>
              ) : (
                qualificationQuestions.map((q, idx) => (
                  <div key={idx}>
                    <p className="font-semibold mb-2">{idx + 1}. {typeof q.question === 'string' ? q.question : ((q.question as any)?.text || JSON.stringify(q.question))}</p>
                    <RadioGroup onValueChange={val => setQualificationAnswers({ ...qualificationAnswers, [idx]: parseInt(val) })}>
                      {q.options.map((opt, oIdx) => (
                        <div key={oIdx} className="flex items-center space-x-2"><RadioGroupItem value={oIdx.toString()} id={`q${idx}-${oIdx}`} /><Label htmlFor={`q${idx}-${oIdx}`}>{typeof opt === 'string' ? opt : ((opt as any)?.text || JSON.stringify(opt))}</Label></div>
                      ))}
                    </RadioGroup>
                  </div>
                ))
              )}
              <Button type="submit" className="w-full" disabled={qualificationQuestions.length === 0}>Submit Test</Button>
            </form>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="w-full max-w-3xl space-y-8 p-8 rounded-xl min-h-[600px] shadow-xl backdrop-blur-sm transition-colors duration-300"
        ...
        {sections.length > 1 && (
          <div className="pb-4 border-b border-gray-100 mb-6">
            <h2 className="text-2xl font-semibold text-primary">{typeof currentSection.title === 'string' ? currentSection.title : ((currentSection.title as any)?.text || JSON.stringify(currentSection.title))}</h2>
          </div>
        )}
            ...
            <div key={question.id} className={`group relative ${isMinimalist ? 'border-b border-gray-100 pb-12' : 'p-6 rounded-lg border border-transparent hover:border-gray-200 hover:bg-white/50'}`}>
              <div className="space-y-3">
                <h3 className="text-xl font-medium" style={{ color: '#000000' }}>
                  {typeof question.text === 'string' ? question.text : (question.text?.text || JSON.stringify(question.text))} {question.required && <span className="text-destructive">*</span>}
                </h3>
                <div className="pt-2">
                  {/* Text Input */}
                  {question.type === 'text' && (
                      ...
                      {question.options?.map((opt: string, idx: number) => {
                        ...
                        return (
                          <div key={idx} onClick={() => setResponses({ ...responses, [question.id]: opt })}
                            ...
                            <span>{typeof opt === 'string' ? opt : ((opt as any)?.text || JSON.stringify(opt))}</span>
                          </div>
                        )
                      })}
                    </div>
