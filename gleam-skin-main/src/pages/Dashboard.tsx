...
const Dashboard = () => {
  ...
  const totalSurveys = totalCount;
  // Note: totalResponses is currently based on the fetched subset (page 1). 
  // Ideally, this should come from a backend stats endpoint.
  const totalResponses = surveys.reduce((acc: number, curr: any) => acc + (curr.response_count || 0), 0);
  const avgResponseRate = surveys.length > 0 ? Math.round(totalResponses / surveys.length) : 0;

  // For "Active Surveys", we'll just count all for this MVP as status isn't fully implemented
  const activeSurveys = totalCount;
                  ...
                  <div key={idx} className="flex items-center justify-between p-4 bg-muted/50 rounded-lg">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">{survey.question_count || 0} Questions • Combined Template</p>
                      </div>
                    </div>
